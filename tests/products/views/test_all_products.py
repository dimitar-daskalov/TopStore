...
class AllProductsTests(TestCase):
    ...
    def test_AllProductsWhenAuthenticatedUser_expectToShowAllProductsSuccessfully(self):
        self.client.force_login(self.user)
        response = self.client.get(reverse('all products'))
        self.assertEqual(200, response.status_code)

    def test_AllProductsWhenNotAuthenticatedUser_expectToShowAllProductsSuccessfully(self):
        response = self.client.get(reverse('all products'))
        self.assertEqual(200, response.status_code)
